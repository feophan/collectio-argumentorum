	...
	.use(function steps() {
		return (tree: Root) => {
			...
			if (!Array.isArray(rootElement.properties.className)) {
				rootElement.properties.className = ['sl-steps', 'after:[&>li:not(:last-of-type)]:bg-gray-300', 'before:[&>li]:bg-gray-100 dark:before:[&>li]:bg-gray-800 before:[&>li]:text-gray-900 dark:before:[&>li]:text-gray-200'];
			} else {
